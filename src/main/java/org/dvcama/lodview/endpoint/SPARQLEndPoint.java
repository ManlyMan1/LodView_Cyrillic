...
public class SPARQLEndPoint {
	...
	public List<TripleBean> doQuery(String IRI, String aProperty, int start, List<String> queries, String filter, String overrideProperty) throws Exception {
		...
		IRI = new String(IRI.getBytes("ISO-8859-1"), "UTF-8");
		...
		for (String query : queries) {
			...
			results = moreThenOneQuery(qe, results, 0, overrideProperty,IRI);
			qe.close();
		}
	...
	private List<TripleBean> moreThenOneQuery(QueryExecution qe, List<TripleBean> results, int retry, String overrideProperty, String IRI) throws Exception {
		org.dvcama.lodview.utils.Misc.WriteToFile(IRI + "  sparql");
		try {
			...
			while (rs.hasNext()) {
				...
				try {
					...
					if (qs.get("s") != null && !qs.get("s").asNode().toString().startsWith("http://")) { // probably
						...
						//org.dvcama.lodview.utils.Misc.WriteToFile("iri="+qs.get("s").asNode().toString() + "sparql");
						...
						org.dvcama.lodview.utils.Misc.WriteToFile("url = "+rb.getUrl()+"iri="+qs.get("s").asNode().toString() + "sparql");
					}
			...
			if (retry < 3) {
				...
				return moreThenOneQuery(qe, results, retry, overrideProperty, IRI);
			}
			ez.printStackTrace();
	...
	public List<TripleBean> doLocalQuery(Model model, String IRI, String localProperty, int start, List<String> queries, String overrideProperty) throws Exception {
		...
		List<TripleBean> results = new ArrayList<TripleBean>();
		// PrintWriter writer = new PrintWriter("/home/narrowbarrow/the-file.txt", "UTF-8");
		// writer.println(IRI);
		// writer.close();
		org.dvcama.lodview.utils.Misc.WriteToFile(IRI + " doLocalQuery");
		for (String query : queries) {
...
//++++++++++++++++++++++++++++
	...
	public String parseQuery(String query, String IRI, String property, int start, String filter) {
		...
		org.dvcama.lodview.utils.Misc.WriteToFile(query+" parseQuery");
		return query;
	}
