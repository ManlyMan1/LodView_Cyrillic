...
public class Misc {
	...
	public static void WriteToFile(String value)
	{
		...
		try(FileWriter writer = new FileWriter("/home/narrowbarrow/IRI.txt", true))
        {
           // запись всей строки
			writer.write(value);
			writer.write("\n");
	...
	public static String Restore(String str) throws UnsupportedEncodingException
	{
	...
	public static String toBrowsableUrl(String value, ConfigurationBean conf) {
		WriteToFile(value+" Util input");
		if (!conf.getPublicUrlSuffix().equals("") && value.startsWith(conf.getIRInamespace())) {
			try {

				WriteToFile(conf.getPublicUrlPrefix() + "?" + conf.getPublicUrlSuffix() + "IRI=" + java.net.URLEncoder.encode(value, "UTF-8")+" Util output");
								
				return conf.getPublicUrlPrefix() + "?" + conf.getPublicUrlSuffix() + "IRI=" + java.net.URLEncoder.encode(value, "UTF-8");
			} catch (UnsupportedEncodingException e) {

			WriteToFile(value+" Util output");
				return value;
			}
		} else {
			WriteToFile(value.replaceAll("^" + conf.getIRInamespace() + "(.+)$", conf.getPublicUrlPrefix() + "$1").replaceAll("([^:])//", "$1/")+" Util output");
			return value.replaceAll("^" + conf.getIRInamespace() + "(.+)$", conf.getPublicUrlPrefix() + "$1").replaceAll("([^:])//", "$1/");
		}

	}

	public static String guessColor(String colorPair, ResultBean r, ConfigurationBean conf) throws UnsupportedEncodingException {
